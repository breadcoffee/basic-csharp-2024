...
namespace NewBookRentalShopApp
{
    public partial class FrmBookInfo : MetroForm
    {
        ...
        private void BtnNew_Click(object sender, EventArgs e)
        {
            ...
            TxtAuthor.Focus(); // 순번은 자동증가하기 때문에 저자에 포커스
        }
        ...
        private void RefreshData()
        {
            using (SqlConnection conn = new SqlConnection(Helper.Common.ConnString))
            {
                ...
                DgvResult.Columns[3].HeaderText = "구분명"; // 구분명(장르) 추가
        ...
        private void DgvResult_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if(e.RowIndex > -1) // 아무것도 선택하지 않으면 -1
            {
                ...
                CboDivision.SelectedValue = selData.Cells[2].Value; // 구분명, 구분코드로 선택해야함
                TxtNames.Text = selData.Cells[4].Value.ToString(); // 책제목
                DtpReleaseDate.Value = DateTime.Parse(selData.Cells[5].Value.ToString()); // 출판일, DateTime.Parse : 문자열을 DateTime으로 형변환
                TxtIsbn.Text = selData.Cells[6].Value.ToString(); // ISBN
                NudPrice.Value = Decimal.Parse(selData.Cells[7].Value.ToString()); // 책가격, Decimal.Parse : 문자열을 숫자형으로 형변환
