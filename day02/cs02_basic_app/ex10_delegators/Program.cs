...
namespace ex10_delegators
{
    ...
    class EventListener // 이벤트가 발생하는지 듣고있는 객체
    {
        ...
        {
            Console.WriteLine($"{name}. 일이 발생했다!! : {message}");
        }
    }
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            // 일반적인 함수 호출에서 한번에 여러개의 함수 실행이 불가능하다. 대리자를 사용해서 가능
            ...
            notifier.EventOccured -= listener2.SomethingHappened; // 리스너 2번은 실행제외
            ...
            notifier.EventOccured("프로그램 시작"); // 2, 3 실행
            Console.WriteLine(); // 윈폼(PyQt 등) 이벤트도 이와 유사한 형태로 동작

            #region "BubbleSort 비교 대리자 코드 영역"
            ...
            var result = Callback(10, 4); // Callback은 calc.Puls를 실행
